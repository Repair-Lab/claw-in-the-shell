...
export default function SetupWizard({ onComplete }) {
  ...
  const [aiSubTab, setAiSubTab] = useState('hermes') // hermes | cloud | local | openclaw
      ...
      <div style={{ flex:1,overflow:'auto',padding:'24px 32px' }}>
        ...
        {step === 3 && (
          <div style={{ maxWidth: 680 }}>
            {/* Sub-Tabs */}
            <div style={{ display:'flex',gap:4,marginBottom:20,borderBottom:'1px solid var(--border)',paddingBottom:8,overflowX:'auto' }}>
              {[{id:'hermes',icon:'🤖',label:'Hermes Agent'},{id:'cloud',icon:'☁️',label:'Cloud-Provider'},{id:'local',icon:'💾',label:'Lokale Modelle'},{id:'openclaw',icon:'🐾',label:'OpenClaw'}].map(st => (
                <button key={st.id} onClick={() => setAiSubTab(st.id)} style={{ padding:'6px 14px',borderRadius:6,border:'none',cursor:'pointer',fontSize:12,fontWeight:aiSubTab===st.id?600:400,background:aiSubTab===st.id?'rgba(0,255,204,0.15)':'transparent',color:aiSubTab===st.id?'var(--accent)':'var(--text-secondary)',transition:'all 0.2s',whiteSpace:'nowrap' }}>
                  {st.icon} {st.label}
                </button>
              ))}
            </div>

            {/* Hermes Agent (NEU — Prominente Sektion) */}
            {aiSubTab === 'hermes' && (
              <div>
                <div style={{ padding:20,background:'linear-gradient(135deg, rgba(0,255,204,0.1) 0%, rgba(0,255,204,0.03) 100%)',border:'2px solid var(--accent)',borderRadius:12,marginBottom:16,boxShadow:'0 0 20px rgba(0,255,204,0.1)' }}>
                  <div style={{ display:'flex',alignItems:'center',gap:12,marginBottom:12 }}>
                    <span style={{ fontSize:40,textShadow:'0 0 20px rgba(0,255,204,0.5)' }}>🤖</span>
                    <div>
                      <div style={{ fontSize:16,fontWeight:700,color:'var(--accent)' }}>Hermes Agent</div>
                      <div style={{ fontSize:11,color:'var(--text-secondary)' }}>Lokaler KI-Operator • Direkte Integration</div>
                    </div>
                  </div>
                  <p style={{ fontSize:12,color:'var(--text-primary)',margin:0,lineHeight:1.6 }}>
                    Hermes ist dein <strong>direkter, lokaler KI-Operator</strong> — kein API-Key erforderlich, 
                    läuft vollständig auf deinem System. Verbinde ihn direkt mit DBAI-OS und nutze deine 
                    lokalen Modelle (Qwen3.8-27B, Qwen3-8B) für alle KI-Aufgaben.
                  </p>
                </div>

                {/* Hermes Provider Card */}
                {providers.filter(p => p.provider_key === 'hermes').map(p => (
                  <div key={p.provider_key} style={{ padding:16,background:'var(--bg-surface)',borderRadius:10,border:'2px solid var(--accent)',marginBottom:16 }}>
                    <div style={{ display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12 }}>
                      <div style={{ display:'flex',alignItems:'center',gap:10 }}>
                        <span style={{ fontSize:24 }}>{p.icon || '🤖'}</span>
                        <div>
                          <div style={{ fontWeight:700,fontSize:14,color:'var(--accent)' }}>{p.display_name}</div>
                    ...
                    <div style={{ display:'flex',gap:6,flexWrap:'wrap',marginBottom:12 }}>
                      ...
                      <span style={{ fontSize:10,padding:'2px 8px',borderRadius:4,background:'rgba(0,255,204,0.15)',color:'var(--accent)',fontWeight:600 }}>KEIN API-KEY</span>
                    </div>

                    <div style={{ display:'flex',gap:8 }}>
                      <button onClick={() => handleTestProvider('hermes')} disabled={testingProvider === 'hermes'} style={{ ...btnPrimary,flex:1 }}>
                        {testingProvider === 'hermes' ? '⏳ Teste…' : '🧪 Verbindung testen'}
                      </button>
                      <button onClick={() => handleSaveProvider('hermes')} style={{ ...btnPrimary,flex:1 }}>
                        💾 Aktivieren
                      </button>
                    </div>

                    {p.is_configured && (
                      <div style={{ marginTop:10,padding:10,background:'rgba(0,255,128,0.1)',border:'1px solid rgba(0,255,128,0.3)',borderRadius:6,fontSize:11,color:'#0f8' }}>
                        ✅ Hermes ist aktiviert und bereit für die Integration!
                      </div>
                    )}
                  </div>
                ))}
                ...
                <div style={{ padding:16,background:'var(--bg-surface)',border:'1px solid var(--border)',borderRadius:10 }}>
                  <div style={{ fontWeight:600,fontSize:12,color:'var(--accent)',marginBottom:10 }}>🚀 Schnellstart</div>
                  <ol style={{ margin:0,padding:'0 0 0 20px',fontSize:11,lineHeight:1.8,color:'var(--text-primary)' }}>
                    <li>Verbindung testen (oben)</li>
                    <li>Hermes aktivieren (Toggle oben)</li>
                    <li>Lokale Modelle scannen (Tab "Lokale Modelle")</li>
                    <li>Qwen-Modelle integrieren</li>
                    <li>Fertig! Hermes nutzt jetzt deine lokalen Modelle</li>
                  </ol>
                </div>
              </div>
            )}
