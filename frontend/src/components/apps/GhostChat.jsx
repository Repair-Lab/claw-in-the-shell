...
export default function GhostChat() {
  ...
  const sendMessage = async () => {
    if ((!input.trim() && pendingImages.length === 0) || loading || !activeChat) return
    ...
    for (const img of pendingImages) {
      ...
      images.push({ type: img.type, data: base64 })
    }
    setPendingImages([])

    // Add user message
    setChatTabs(prev => prev.map(t => t.id === activeChatId
      ? { ...t, messages: [...t.messages, { role: 'user', content: userMsg, images: images, time: new Date() }] }
      : t
    ))

    setLoading(true)
    try {
      const result = await api.askGhost(activeChat.role, userMsg, {}, selectedModel || null, images)
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                  ...
                  <div style={{ display: 'flex', gap: 4, marginBottom: 6, flexWrap: 'wrap' }}>
                    {msg.images.map((img, j) => (
                      <img
                        ...
                        alt=""
                        style={{ maxWidth: 120, maxHeight: 120, borderRadius: 4, border: '1px solid #2a2a40' }}
                      />
                    ))}
                  </div>
                )}
          ...
          <div style={sx.inputBar}>
            ...
            {pendingImages.length > 0 && (
              <div style={sx.imagePreviewStrip}>
                {pendingImages.map((img, i) => (
                  <div key={i} style={sx.imageThumb}>
                    <img src={img.preview} alt={img.name} style={sx.imageThumbImg} />
                    <button onClick={() => removeImage(i)} style={sx.imageRemoveBtn} title="Entfernen">×</button>
                  </div>
                ))}
              </div>
            )}
            <div className="chat-input" style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 4 }}>
              <input
                ref={fileInputRef}
                type="file"
                accept="image/*"
              ...
              <button
                ...
                disabled={loading}
                style={sx.imageAttachBtn}
                title="Bild anhängen (Vision/MML)"
              >📎</button>
              <input
                value={input}
...
const sx = {
  ...
  // ── Image Upload ──
  imagePreviewStrip: {
    display: 'flex', gap: 6, padding: '6px 8px', flexWrap: 'wrap',
  },
  imageThumb: {
    position: 'relative', width: 60, height: 60, borderRadius: 6,
    overflow: 'hidden', border: '1px solid #2a2a40',
  },
  imageThumbImg: {
