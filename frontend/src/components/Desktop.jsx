...
import GhostPlay from './apps/GhostPlay'
...
export default function Desktop({ user, desktopState, tabInfo, onLogout }) {
  ...
  // ── Standard-Media-Handler: `dbai:open_media` öffnet Medien in GhostPlay ──
  // detail: { url, type?, title? }  → wie ein Datei-Doppelklick unter OSX/Windows
  useEffect(() => {
    const handler = (e) => {
      const { url, type, title } = e.detail || {}
      if (!url) return
      openApp('ghostplay', { mediaUrl: url, mediaType: type, title })
    }
    window.addEventListener('dbai:open_media', handler)
    return () => window.removeEventListener('dbai:open_media', handler)
  }, [openApp])
